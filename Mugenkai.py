...
class Library:
    ...
    def list_games_obj(self):
        """Returns a list of all games objects in the library."""
        return self.games
...
def filterListbox():
    """A filter to search the current selected library"""
    search_text = dpg.get_value("GameFilter").lower()
    Current_Selected_Library = (manager.search_libraries(dpg.get_value("LibrariesCombo")))[0]
    listbox_items = Current_Selected_Library.list_games_obj()  # Get the list of items
    
    filtered_items = [
        item.title for item in listbox_items if
        search_text in item.title.lower() or  # Search in title
        search_text in item.genre.lower() or  # Search in genre
        search_text in item.abbreviation.lower()  # Search in abbreviation
    ]
    dpg.configure_item(item="GameList", items=filtered_items) # Update the listbox
...
with dpg.window(label="Launcher Main Menu",tag="MainWindow",width=ViewPortWidth,height=ViewPortHeight,no_close=True,no_collapse=True,no_move=True,no_open_over_existing_popup=True,no_focus_on_appearing=True,no_bring_to_front_on_focus=True,no_resize=True,no_title_bar=True):
    ...
    with dpg.group(horizontal=True,parent="MainWindow"):
        with dpg.child_window(autosize_x=False,height=-1,width=350,tag="GamesWindowList"):
            ...
            with dpg.group(horizontal=True):
                dpg.add_text("Search:")
                dpg.add_input_text(width=-1, callback=filterListbox, tag="GameFilter")

            with dpg.group(horizontal=True):
                First_Library = (manager.search_libraries(dpg.get_value("LibrariesCombo")))[0]
                dpg.add_listbox(items=(First_Library.list_games()), width=350, callback=SelectedGame, num_items=First_Library.list_int_games()+3, tag="GameList")
